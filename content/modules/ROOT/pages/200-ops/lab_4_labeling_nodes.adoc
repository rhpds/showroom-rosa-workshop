...
. ROSA does not automatically update the labels on existing nodes, you can add the label to existing nodes manually by running the following command:
+
[source,sh,role=execute]
----
oc label nodes -l "hypershift.openshift.io/nodePool=rosa-${GUID}-workers" tier=frontend
----
+
.Sample Output
[source,text,options=nowrap]
----
node/ip-10-0-0-113.us-east-2.compute.internal labeled
node/ip-10-0-0-164.us-east-2.compute.internal labeled
node/ip-10-0-0-210.us-east-2.compute.internal labeled
node/ip-10-0-0-42.us-east-2.compute.internal labeled
----
